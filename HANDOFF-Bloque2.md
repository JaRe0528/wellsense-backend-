...
## Actualización 9 — CIERRE de Bloque 2: 2 bugs adicionales encontrados y corregidos (validado con `dotnet test` real: 22/22)

Mi Actualización 8 decía "con esto deberían pasar los 22/22" — resultó
incompleto. Otro proceso los diagnosticó y corrigió mientras tanto; recibí
el ZIP con ambos fixes ya aplicados y **validado con `dotnet test` real**
(no solo compilación) — sincronicé mi copia de trabajo contra ese ZIP y
confirmé que ambos archivos quedaron exactamente así, sin ninguna referencia
residual a la versión anterior de ninguno de los dos. Los dejo registrados
aquí para que este documento sea el reflejo fiel del código que ya está en
el repo, no de lo que yo mismo escribí antes de que se encontraran estos dos:

**1) `CustomWebApplicationFactory.cs` — el store de InMemory se reconstruía
vacío en cada request.** El `ServiceProvider` interno de EF InMemory
(`new ServiceCollection().AddEntityFrameworkInMemoryDatabase().BuildServiceProvider()`)
se armaba **dentro** del callback `options => {...}` que se le pasa a
`AddDbContext`. Ese callback se re-ejecuta una vez por cada scope — y
`AddDbContext` registra `DbContextOptions<TContext>` con lifetime *Scoped*
por defecto, así que "cada scope" es, en la práctica, cada request HTTP.
Resultado: aunque `DbName` (el string) fuera el mismo en todas las
llamadas, cada request construía su propio `ServiceProvider` interno nuevo,
y por lo tanto su propio store de InMemory vacío — un registro hecho en
`/register` no sobrevivía hasta el `/verify-email` o `/login` del mismo
flujo, porque cada uno corría contra un store en memoria distinto. Fix:
el `ServiceProvider` ahora se construye **una sola vez**, en una variable
capturada **afuera** del callback de `AddDbContext`, y el callback lo
reutiliza vía closure — mismo store en cada request.

**2) `Program.cs` — mismatch entre la clave con la que se firmaba el JWT y
la clave con la que se validaba.** `Jwt:Secret` se leía en una variable
local `var jwtSecret = builder.Configuration["Jwt:Secret"]` **antes** de
`builder.Build()`. El problema: `WebApplicationFactory` (usado en las
pruebas de integración) aplica sus overrides de configuración
(`ConfigureAppConfiguration`, incluyendo el `Jwt:Secret` de prueba) recién
al reconstruir el host — es decir, **después** de que ese código de nivel
superior ya había corrido y capturado el valor real de `appsettings.json`
(el placeholder). Mientras tanto, `JwtTokenService` (quien firma los
tokens) lee `Jwt:Secret` vía `IConfiguration` inyectado en tiempo de
request, viendo sí la configuración ya fusionada con el override de
prueba. Resultado: el token se firmaba con la clave de prueba pero se
validaba contra la clave del placeholder → firma "inválida" → 401 en
cualquier endpoint protegido, incluso con un token recién emitido y
legítimo. Fix: la lectura de `Jwt:Secret` (y de `Jwt:Issuer`/`Jwt:Audience`)
se movió **dentro** del callback de `AddJwtBearer(options => {...})`, que
ASP.NET Core invoca de forma perezosa, después de `Build()` — mismo patrón
que ya usaba `DeviceLinkCodeHasher` con `DeviceLink:Pepper`.

**Estado de Bloque 2 con esto: CERRADO.** 22/22 pruebas pasando, validado
con `dotnet test` real (no solo `dotnet build`) — el riesgo abierto #1 de
este documento ("nada de esto está compilado/probado por mí") queda resuelto
para el ZIP que ya está en el repo, aunque como práctica general yo sigo sin
poder ejecutar `dotnet` en mi propio entorno de trabajo.
...
## 0. Aviso operativo (se mantiene del Bloque 1 — ver estado final en Actualización 9)

**Estado final: superado.** Todo este aviso describe la limitación tal como
existía durante la mayor parte de este bloque (9 rondas de ida y vuelta
corrigiendo errores de compilación y runtime uno por uno). Con la
Actualización 9, el ZIP que queda en el repo ya fue compilado y probado de
verdad con `dotnet build && dotnet test` (22/22), fuera de mi entorno de
trabajo. Dejo el resto de esta sección tal cual para que el historial de
por qué se trabajó así quede completo.
...
1. ~~Nada de esto está compilado por mí~~ — **resuelto**: el ZIP final
   (Actualización 9) está validado con `dotnet build && dotnet test` real,
   22/22 pruebas pasando. Sigue siendo cierto que yo, en mi propio entorno de
   trabajo, no puedo ejecutar `dotnet` — pero el código que queda en el
   repo ya fue compilado y probado de verdad por ustedes.
2. **Pruebas de integración HTTP usan EF InMemory, no Postgres real**
   (Testcontainers) — decisión documentada en §3.1, sigue siendo válida:
   estas pruebas verifican el pipeline HTTP (JWT, rate limiting, manejo de
   errores), no el comportamiento de índices únicos/parciales reales de
   Postgres (eso ya está cubierto aparte, en HANDOFF-DB y el Bloque 1). Si
   en algún momento quieren la versión con Postgres real, es un cambio
   acotado a `CustomWebApplicationFactory.cs`.
...
4. ~~Decisión de bloquear login sin email verificado~~ y
   ~~revocar todos los refresh tokens en reset/change password~~ —
   **confirmadas correctas por el usuario, no se tocan.**
5. **`attempts`/`max_attempts` sin uso funcional real** en el flujo elegido
...
6. **`IConfiguration` crudo en Application** (Login/RefreshToken/
   ...
   Infrastructure — sigue sin hacerse, no bloqueante.
...
- [x] Pruebas de integración HTTP end-to-end (JWT bearer real, rate limiting
      real) — ver §3.1
- [x] Compilación y `dotnet test` reales — **22/22 pruebas pasando**,
      confirmado (Actualización 9)

**Bloque 2: CERRADO.** Arrancando Bloque 3 (Users + Profile, incluida la
decisión de zona horaria para `wellness_scores`/`stress_scores`) en el
siguiente HANDOFF.
