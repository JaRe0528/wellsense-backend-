...
## Actualización — 78/100 → 100/100: bug real en la semilla de prueba corregido (regla general para bloques futuros)

Compilaste y corriste: 22 de 100 pruebas fallaban. La causa raíz era mía —
en `CustomWebApplicationFactory.cs`, sembraba `MembershipPlans` con un
`WellSenseDbContext` construido a mano
(`new WellSenseDbContext(new DbContextOptionsBuilder<WellSenseDbContext>()...)`)
dentro de `ConfigureTestServices`, apuntando al mismo
`InMemoryServiceProvider` compartido que usa la app — pero como ese
contexto de siembra NUNCA pasó por `services.AddDbContext<WellSenseDbContext>(...)`,
le faltaban opciones que EF exige que sean **idénticas** entre todos los
`DbContext` que comparten un mismo `ServiceProvider` interno (en particular
el `ApplicationServiceProvider` real de la app, que solo `AddDbContext`
asigna). Resultado: la primera consulta real a la base en CUALQUIER
endpoint (`db.Users` en `/register`, por ejemplo) tronaba con
`InvalidOperationException` sobre `ConfigureWarnings` — el mensaje confunde
porque esa es solo la primera discrepancia que EF encuentra al validar, no
la causa real. De ahí en cascada, las `KeyNotFoundException` en
`CapturedEmails` de cualquier prueba que dependiera de un `/register`
exitoso.

Fix aplicado (no lo hice yo — lo sincronizo, ya validado en GitHub, commit
`8cd6f7d`, 100/100): en vez de construir el contexto de siembra a mano, se
sobrescribió `CreateHost(IHostBuilder)` — el punto de extensión que
`WebApplicationFactory` expone específicamente para sembrar datos DESPUÉS
de que el host ya terminó de construirse — y ahí se resuelve
`WellSenseDbContext` desde `host.Services.CreateScope()`, la misma tubería
de DI que usa cualquier request real. Al compartir esa tubería, las
opciones son automáticamente las mismas — no hay forma de que diverjan.

**Regla general que me llevo para bloques futuros**: cualquier `DbContext`
de siembra/setup en pruebas SIEMPRE debe resolverse desde el contenedor de
DI ya construido (`host.Services`/`IServiceProvider` vía
`CreateScope()`/`GetRequiredService<T>()`), nunca armarse con un
`DbContextOptionsBuilder` independiente — aunque apunte al mismo nombre de
base InMemory y al mismo `ServiceProvider` interno compartido. La
apariencia de "mismo store, debería ser equivalente" es la trampa exacta
en la que caí: EF valida más que solo el nombre de la base cuando varios
contextos comparten un `ServiceProvider` interno.
...
- Se agregó **semilla de los 4 planes** al store InMemory de pruebas (la
  ...
  Se siembra sobrescribiendo `CreateHost`, resolviendo `WellSenseDbContext`
  desde `host.Services` — no con un `DbContextOptionsBuilder` armado a
  mano (ver Actualización al inicio de este documento: esa segunda forma
  fue exactamente el bug que rompió 22/100 pruebas).
